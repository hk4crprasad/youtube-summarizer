...
_token_timeout = 1800
# Default location in local/dev
_fallback_dir = pathlib.Path(__file__).parent.resolve() / '__cache__'
_fallback_token_file = os.path.join(_fallback_dir, 'tokens.json')

# Cloud-safe location
_tmp_token_file = '/tmp/tokens.json'

# Use /tmp if it exists (written by /paste endpoint), fallback otherwise
_token_file = _tmp_token_file if os.path.exists(_tmp_token_file) else _fallback_token_file
